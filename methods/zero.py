...
class Zero(L.LightningModule):
    ...
    def step_g(self, batch):
        ...
        gan_loss = self.positive_loss(
            self.discriminator(torch.cat([predict,  F.interpolate(crushed, scale_factor=4, mode="bilinear")], dim=1)),
            self.discriminator(torch.cat([original, F.interpolate(crushed, scale_factor=4, mode="bilinear")], dim=1)).detach()
        )

        self.log("Gan Loss", gan_loss, on_step=True)

        return crushed, original, predict, gan_loss 
    ...
    def configure_optimizers(self):
        ...
        return [
            ...
            torch.optim.AdamW(self.discriminator.parameters(), lr=1e-5, betas=(0.9, 0.95), weight_decay=0.05, amsgrad=True)
        ], []
